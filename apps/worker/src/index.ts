...
type Env = {
  DB: D1Database;
  // EMAIL is optional until Cloudflare Email Routing is provisioned on a domain.
  EMAIL?: SendEmail;
  SERPAPI_KEY: string;
  EMAIL_TO?: string;
  EMAIL_FROM?: string;
...
async function sendAlertDigest(env: Env, alerts: Array<{ snapshot: FareSnapshot; averagePrice: number; dropPercent: number }>) {
  ...
  // Email Routing is not yet provisioned; log payload and exit gracefully when
  // EMAIL/EMAIL_FROM/EMAIL_TO are absent so cron execution always completes.
  const emailReady =
    env.EMAIL &&
    typeof env.EMAIL.send === "function" &&
    typeof env.EMAIL_FROM === "string" &&
    env.EMAIL_FROM.length > 0 &&
    typeof env.EMAIL_TO === "string" &&
    env.EMAIL_TO.length > 0;

  if (!emailReady) {
    console.log(`[alert-digest] EMAIL binding or addresses unavailable; logging payload instead. Subject: ${subject}\n${body}`);
    return;
  }

  try {
    const message = new EmailMessage(
      env.EMAIL_FROM as string,
      env.EMAIL_TO as string,
      `Subject: ${subject}\r\nContent-Type: text/plain; charset=UTF-8\r\n\r\n${body}`
    );
    await (env.EMAIL as SendEmail).send(message);
  } catch (error) {
    console.log(`[alert-digest] Email send failed; logging payload instead. Error: ${String(error)}. Subject: ${subject}\n${body}`);
  }
}
